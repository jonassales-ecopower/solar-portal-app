...
10. DATAS DE LEITURA: Extraia as datas exatas do campo "Datas de Leituras" ou "Leitura Anterior / Leitura Atual".
    Procure também por campos como "Leitura Anterior:", "Leitura Atual:", "Data de Leitura".
    Exemplo: "Leitura Anterior: 13/03/2026 | Leitura Atual: 13/04/2026"
    → data_leitura_anterior = "13/03/2026", data_leitura_atual = "13/04/2026"

11. PRÓXIMA LEITURA: Extraia a data do campo "Próxima Leitura" se existir.
    Procure por "Próxima Leitura", "Próx. Leitura", "Next Reading".
    Exemplo: "Próxima Leitura: 14/05/2026" → data_proxima_leitura = "14/05/2026"
...
{{
  ...
  "data_leitura_anterior": "DD/MM/AAAA",
  "data_leitura_atual": "DD/MM/AAAA",
...
def calcular_analise_consumo(dados: dict, geracao_total_kwh: float = None) -> dict:
    ...
    if geracao_total_kwh and geracao_total_kwh > 0:
        ...
        resultado["consumo_total_kwh"] = round(consumo_instantaneo + consumo_rede, 2)
    else:
...
def foxess_get_mensal(api_key: str, serial: str, ano: int, mes: int) -> float:
    """Retorna geração total REAL do mês completo (01 ao último dia do mês)."""
...
def foxess_get_periodo(api_key: str, serial: str, data_inicio: date, data_fim: date) -> float:
    """
    Retorna geração total do PERÍODO EXATO da conta (data_leitura_anterior a data_leitura_atual).
    Usado apenas para o balanço energético da conta — NÃO para o gráfico mensal.
    Usa dimension=month e filtra apenas os dias do período por índice.
    """
    total = 0.0
    dias_por_mes = {}
    ...
    while data_atual <= data_fim:
        ...
        if mes_ano not in dias_por_mes:
            dias_por_mes[mes_ano] = []
        dias_por_mes[mes_ano].append(data_atual.day)
        data_atual += timedelta(days=1)

    for (ano, mes), dias in dias_por_mes.items():
        body = {"sn": serial, "year": ano, "month": mes, "dimension": "month", "variables": ["generation"]}
        try:
            r = foxess_chamar_api(api_key, "op/v0/device/report/query", body)
            if r.get("errno") == 0:
...
def salvar_geracao_mensal(cliente_id: int, ano: int, mes: int, geracao_kwh: float, fonte: str = "foxess"):
    """Salva o total mensal COMPLETO (01 ao último dia do mês) — usado pelo gráfico."""
    conn = conectar_banco()
    cur = conn.cursor()
    cur.execute("""
...
def buscar_geracao_periodo_banco(cliente_id: int, data_inicio, data_fim):
    conn = conectar_banco()
    cur = conn.cursor()
...
def salvar_no_banco(dados, cliente_id):
    ...
    try:
        if dados.get("data_leitura_anterior"):
            data_leit_ant = datetime.strptime(dados["data_leitura_anterior"], "%d/%m/%Y").date()
        if dados.get("data_leitura_atual"):
            data_leit_atu = datetime.strptime(dados["data_leitura_atual"], "%d/%m/%Y").date()
        if dados.get("data_proxima_leitura"):
            data_prox_leit = datetime.strptime(dados["data_proxima_leitura"], "%d/%m/%Y").date()
    except:
        pass
    cur.execute("""
        INSERT INTO contas (
    ...
    """, (
        ...
        data_leit_ant, data_leit_atu, data_prox_leit
    ))
...
def listar_contas(cliente_id: int, integrador: dict = Depends(obter_integrador_atual)):
    ...
    cur.execute("""
        SELECT c.id,c.mes_referencia,c.data_vencimento,c.consumo_kwh,c.energia_injetada_kwh,
               ...
               c.leitura_por_media,c.meses_acumulados,c.data_leitura_anterior,
               c.data_leitura_atual,c.data_proxima_leitura
    ...
    return [{"id":c[0],"mes_referencia":c[1],"data_vencimento":str(c[2]),"consumo_kwh":float(c[3] or 0),
             ...
             "criado_em":str(c[15]),"leitura_por_media":c[16],"meses_acumulados":c[17],
             "data_leitura_anterior":str(c[18]) if c[18] else None,
             "data_leitura_atual":str(c[19]) if c[19] else None,
             "data_proxima_leitura":str(c[20]) if c[20] else None} for c in contas]
...
def listar_contas_publico(cliente_id: int):
    ...
    cur.execute("""
        SELECT id,mes_referencia,data_vencimento,consumo_kwh,energia_injetada_kwh,
               ...
               leitura_por_media,meses_acumulados,data_leitura_anterior,
               data_leitura_atual,data_proxima_leitura
    ...
    return [{"id":c[0],"mes_referencia":c[1],"data_vencimento":str(c[2]),"consumo_kwh":float(c[3] or 0),
             ...
             "leitura_por_media":c[18],"meses_acumulados":c[19],
             "data_leitura_anterior":str(c[20]) if c[20] else None,
             "data_leitura_atual":str(c[21]) if c[21] else None,
             "data_proxima_leitura":str(c[22]) if c[22] else None} for c in contas]
...
async def upload_conta(cliente_id: int, arquivo: UploadFile = File(...)):
    ...
    try:
        ...
        if inversor and inversor[0] and inversor[0].lower() == "foxess" and inversor[2]:
            try:
                data_leit_ant_str = dados.get("data_leitura_anterior")
                data_leit_atu_str = dados.get("data_leitura_atual")
                ...
                if data_leit_ant_str and data_leit_atu_str:
                    # 1. Geração do PERÍODO EXATO → usado no balanço energético da conta
                    data_ini = datetime.strptime(data_leit_ant_str, "%d/%m/%Y").date()
                    data_fim = datetime.strptime(data_leit_atu_str, "%d/%m/%Y").date()
                    g_periodo = foxess_get_periodo(inversor[2], inversor[1], data_ini, data_fim)
                    if g_periodo > 0:
                        geracao_total_kwh = g_periodo

                    # 2. Geração do MÊS COMPLETO → usado no gráfico (separado!)
                    if mes_num:
                        g_mensal = foxess_get_mensal(inversor[2], inversor[1], ano_num, mes_num)
                        if g_mensal > 0:
                            salvar_geracao_mensal(cliente_id, ano_num, mes_num, g_mensal)
                else:
                    # Fallback sem datas de leitura
                ...
                # Salvar próxima leitura no cadastro do cliente
                if dados.get("data_proxima_leitura"):
                    try:
                        prox_leit = datetime.strptime(dados["data_proxima_leitura"], "%d/%m/%Y").date()
                        conn2 = conectar_banco()
                        cur2 = conn2.cursor()
                        cur2.execute("UPDATE clientes SET proxima_leitura=%s WHERE id=%s", (prox_leit, cliente_id))
                        conn2.commit()
                        cur2.close()
                        conn2.close()
                    except Exception:
...
def verificar_anomalias(cliente_id: int):
    ...
    ultimos7 = buscar_geracao_periodo_banco(cliente_id, hoje-timedelta(days=8), ontem)
