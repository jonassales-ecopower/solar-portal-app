...
def foxess_chamar_api(api_key: str, path: str, body: dict):
    ...
    # Path com barra inicial para a assinatura
    path_com_barra = f"/{path}"
    signature_raw = f"{path_com_barra}\r\n{api_key}\r\n{timestamp}"
...
def monitoramento_foxess(cliente_id: int):
    ...
    try:
        ...
        body = {
            "sns": [serial],
            "variables": ["pvPower", "feedinPower", "gridConsumptionPower", "loadsPower", "generationPower"]
               }
        data = foxess_chamar_api(api_key, path, body)
        ...
        return {
            ...
            "serial": serial,
            "status": "online",
            "geracao_atual_kw": float(variaveis.get("pvPower", 0) or 0),
            "injetado_rede_kw": float(variaveis.get("feedinPower", 0) or 0),
            "consumo_rede_kw": float(variaveis.get("gridConsumptionPower", 0) or 0),
            "consumo_casa_kw": float(variaveis.get("loadsPower", 0) or 0),
            "potencia_total_kw": float(variaveis.get("generationPower", 0) or 0),
